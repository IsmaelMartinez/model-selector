 ...
 * Implements multi-method classification with improved accuracy and confidence scoring
 */
...
export class EnhancedTaskClassifier {
  ...
  async classify(taskDescription, options = {}) {
    ...
    try {
      // Method 1: OSS Model Classification (if available)
      if (!options.offline && options.useOSSModel) {
        const ossResult = await this.classifyWithOSSModel(taskDescription);
        if (ossResult.success && ossResult.confidence > 0.7) {
          result.predictions = ossResult.predictions;
          result.subcategoryPredictions = ossResult.subcategoryPredictions;
          result.method = 'oss_model';
          result.confidence = ossResult.confidence;
          result.confidenceLevel = this.getConfidenceLevel(ossResult.confidence);
          this.stats.methodUsage.api++;
          return this.finalizeResult(result, startTime);
        }
      }

      // Method 2: Hugging Face API Zero-shot Classification
      ...
      // Method 3: Enhanced Semantic Similarity
      ...
      // Method 4: Enhanced Keyword Classification
  ...
  /**
   * Method 1: OSS Model Classification (placeholder for future implementation)
   */
  async classifyWithOSSModel(taskDescription) {
    // Placeholder for OSS model integration
    // Could integrate with models like sentence-transformers, BERT-based classifiers
    console.log('🤖 Attempting OSS model classification...');
    
    // For now, return unsuccessful to fall back to other methods
    return { success: false, reason: 'OSS model not implemented' };
    
    /* Future implementation could use:
     * - Sentence transformers for embedding similarity
     * - Local BERT model for zero-shot classification
     * - Custom trained classification model
     */
  }

  /**
   * Method 2: Enhanced Hugging Face API Classification
   */
  ...
  /**
   * Method 3: Enhanced Semantic Similarity Classification
   */
  ...
  calculateSemanticSimilarity(text, keyword) {
    ...
    // Edit distance similarity for partial matches
    const editSimilarity = 1 - (this.levenshteinDistance(text, keyword) / Math.max(text.length, keyword.length));
  ...
  /**
   * Get subcategory predictions based on category predictions
   */
  async getSubcategoryPredictions(taskDescription, categoryPredictions) {
    ...
    for (const prediction of categoryPredictions.slice(0, 2)) {
      ...
      const subcategoryLabels = Object.entries(categoryData.subcategories).map(([key, data]) => data.label);
        ...
        try {
          const response = await fetch('https://api-inference.huggingface.co/models/facebook/bart-large-mnli', {
            ...
            body: JSON.stringify({
              inputs: taskDescription,
              parameters: { 
                candidate_labels: subcategoryLabels,
                multi_label: false
              }
            })
          });
          ...
          if (response.ok) {
            ...
            for (let i = 0; i < Math.min(2, data.labels.length); i++) {
              const subcategoryKey = Object.entries(categoryData.subcategories).find(([key, subData]) => 
                subData.label === data.labels[i]
              )?.[0];
              
              if (subcategoryKey) {
                subcategoryPredictions.push({
                  category: prediction.category,
                  subcategory: subcategoryKey,
                  label: data.labels[i],
                  score: data.scores[i] * prediction.score // Weight by category confidence
                });
              }
            }
          }
        } catch (error) {
          // Fall back to keyword-based subcategory prediction
          console.log('Falling back to keyword-based subcategory prediction');
        }
      }
    }
  ...
  normalizePredictions(categoryScores) {
    ...
    if (totalScore > 0) {
      predictions.forEach(p => p.score = p.score / totalScore);
    }
   ...
   */
  async batchClassify(descriptions, options = {}) {
    const results = [];
    
    for (const description of descriptions) {
      const result = await this.classify(description, options);
      results.push(result);
      
      // Rate limiting for API calls
      if (result.method.includes('api') && descriptions.length > 1) {
...
// Export enhanced classifier
export const enhancedTaskClassifier = new EnhancedTaskClassifier();