# Documentation

## Guides

- [User Guide](user-guide.md) - How to use the app

## Technical

- [Data Structure](data-structure.md) - Tasks and models schema
- [Environmental Methodology](environmental-methodology.md) - Scoring method
- [Model Curation](model-curation-process.md) - How models are curated
- [Auto Updates](auto-updates.md) - Automated update workflow

## Architecture Decisions

- [ADR-0001](adrs/adr-0001-sveltekit-vite-rules-first-mvp.md) - Tech stack (SvelteKit + Vite)
- [ADR-0002](adrs/adr-0002-task-classification-model-selection.md) - Task classification approach
- [ADR-0003](adrs/adr-0003-browser-llm-classification-model-selection.md) - Browser LLM selection
- [ADR-0004](adrs/adr-0004-slm-model-selection.md) - SLM evaluation (rejected)
- [ADR-0005](adrs/adr-0005-automated-model-dataset-updates.md) - Automated updates
- [ADR-0006](adrs/adr-0006-model-accuracy-filtering.md) - Accuracy filtering
- [ADR-0007](adrs/adr-0007-ensemble-classification-mode.md) - Ensemble classification
- [ADR-0008](adrs/adr-0008-embedding-similarity-classification.md) - Embedding classification

## Research

- [Browser SLM Learnings](research/browser-slm-learnings.md) - SLM technical evaluation
- [Model Specialization Patterns](research/model-specialization-patterns.md) - Why smaller models specialize
- [RAG Classification Evaluation](research/rag-classification-evaluation.md) - RAG vs embedding approach
- [Multi-Persona Debate Findings](research/multi-persona-debate-findings.md) - System design analysis
